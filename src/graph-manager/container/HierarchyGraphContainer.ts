...
export class HierarchyGraphContainer<
...
>
  ...
  protected addChildren(
    ...
    for (const child of children) {
      this.relations[current.key].push(child.current);
      this.relations[current.key] = this.relations[current.key].concat(
        this.addChildren(child.current, child.children),
      );
    }
